...
import {
  IBoardGame,
  IBoardGameData,
  IBoardGameUserFirestore,
} from '@/features/boardgames/types';
  ...
  useEffect(() => {
    ...
    const unsubscribeGames = onSnapshot(
      ...
      (querySnapshot) => {
        ...
        const gamesData: IBoardGameData[] = querySnapshot.docs.map(
          (doc) =>
            ({
              id: doc.id,
              ...doc.data(),
        ...
        unsubscribeUserGames = onSnapshot(
          qUserGames,
          (userQuerySnapshot) => {
            if (!isMounted) return;

            const userGamesData: IBoardGameUserFirestore[] =
              userQuerySnapshot.docs.map(
                (doc) => doc.data() as IBoardGameUserFirestore,
              );

            const combinedGames: IBoardGame[] = gamesData.map((game) => {
              const allUserDataForGame = userGamesData.filter(
                (ug) => ug.boardGameId === game.id,
              );
